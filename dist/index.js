...
class PDFFormsFiller {
    constructor(formTemplatePath, outputFilePath) {
     ...
     */
    startModifiedDictionary(handles, originalDict, excludedKeys) {
    ...
    writeAppearanceXObjectForText(handles, formId, fieldsDictionary, text, inheritedProperties) {
        ...
        const xobjectForm = this.writer.createFormXObject(0, 0, rect[2].value - rect[0].value, rect[3].value - rect[1].value, formId);
    ...
    writeFieldWithAppearanceForText(handles, targetFieldDict, sourceFieldDictionary, appearanceInField, textToWrite, inheritedProperties) {
    ...
    updateTextValue(handles, fieldDictionary, value, isRich, inheritedProperties) {
    ...
    */
    updateFieldWithValue(handles, fieldDictionary, value, inheritedProperties) {
    ...
    */
    writeFilledField(handles, fieldDictionary, inheritedProperties, baseFieldName) {
    ...
    */
    writeKidsAndEndObject(handles, parentDict, kidsArray) {
    ...
    */
    writeFilledFields(handles, parentDict, fields, inheritedProperties, baseFieldName) {
        ...
        fieldsReferences.forEach((fieldReference) => {
            if (fieldReference.existing) {
                handles.objectsContext.startModifiedIndirectObject(fieldReference.id);
                this.writeFilledField(handles, handles.reader.parseNewObject(fieldReference.id).toPDFDictionary(), inheritedProperties, baseFieldName);
            }
            else {
                handles.objectsContext.startNewIndirectObject(fieldReference.id);
